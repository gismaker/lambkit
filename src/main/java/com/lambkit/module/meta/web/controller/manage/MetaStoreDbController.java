 ...
 * @since 1.0
 */
public class MetaStoreDbController extends LambkitController {
	private final String tableName = "meta_store_db";
	...
	public void index() {
		...
		if(id!=null) {
			setAttr("model", MetaStoreDb.service().findById(id));
			setAttr("tables", MetaTable.service().find(MetaTable.sql().andSidEqualTo(id).example()));
		}
		keepPara();
	...
	public void list() {
		...
		setAttr("dbs", MetaStoreDb.service().find(MetaStoreDb.sql().example()));
		keepTable(tbc);
		keepPara();
	...
	public void save() {
		MetaStoreDb app = getModel(MetaStoreDb.class, "model");
		app.save();
	...
	public void delete() {
		Long id = getParaToLong(0);
		if(id!=null) {
			MetaStoreDb.service().deleteById(id);
		}
		renderJson(new AjaxResult(1, "success", null));
	}
