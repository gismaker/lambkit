 ...
 * meta_field_map标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:meta_field_map信息}
 * @author lambkit
 */
public class MetaFieldMapDirective extends LambkitTemplateModel {
	...
	public void onRender(Environment env, Map params, TemplateModel[] loopVars,
		...
		String mtype = get(params, "mtype");
		String srid = get(params, "srid");
		...
		String sql = " from meta_field_map where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			if(StringUtils.hasText(id)) sql += " and id=" + id;//int unsigned
			if(StringUtils.hasText(fldid)) sql += " and fldid=" + fldid;//int unsigned
			if(StringUtils.hasText(mtype)) sql += " and mtype like '%" + mtype + "%'";//varchar
			if(StringUtils.hasText(srid)) sql += " and srid like '%" + srid + "%'";//varchar
		...
		MetaFieldMapService service = MetaFieldMap.service();
