 ...
 */
/**
 * meta_file_catalog_mapping标签<br>
 * 参数：{id:主键}
...
public class MetaFileCatalogMappingDirective extends LambkitTemplateModel {
	...
	public void onRender(Environment env, Map params, TemplateModel[] loopVars,
		...
		String sql = " from meta_file_catalog_mapping where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			if(StringUtils.hasText(catalogId)) sql += " and catalog_id=" + catalogId;//int unsigned
		...
		MetaFileCatalogMappingService service = MetaFileCatalogMapping.service();
