 ...
 * meta_theme标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:meta_theme信息}
 * @author lambkit
 */
public class MetaThemeDirective extends LambkitTemplateModel {
	...
	public void onRender(Environment env, Map params, TemplateModel[] loopVars,
		...
		String tmtype = get(params, "tmtype");
		String active = get(params, "active");
		int pagenum = getInt(params, "pagenum", 0);
		int pagesize = getInt(params, "pagesize", 0);
		String wheresql = get(params, "sql", null);
		String sql = " from meta_theme where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			if(StringUtils.hasText(id)) sql += " and id=" + id;//int unsigned
			if(StringUtils.hasText(name)) sql += " and name like '%" + name + "%'";//varchar
			if(StringUtils.hasText(userid)) sql += " and userid=" + userid;//int unsigned
			if(StringUtils.hasText(tmtype)) sql += " and tmtype like '%" + tmtype + "%'";//varchar
			if(StringUtils.hasText(active)) sql += " and active=" + active;//int
		} else {
			sql += wheresql;
		}
		...
		MetaThemeService service = MetaTheme.service();
