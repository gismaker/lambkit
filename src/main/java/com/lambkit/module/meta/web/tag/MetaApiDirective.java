 ...
 * meta_api标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:meta_api信息}
 * @author lambkit
 */
public class MetaApiDirective extends LambkitTemplateModel {
	...
	public void onRender(Environment env, Map params, TemplateModel[] loopVars,
		...
		if(wheresql == null) {
			...
			if(StringUtils.hasText(action)) sql += " and action like '%" + action + "%'";//varchar
			if(StringUtils.hasText(format)) sql += " and format like '%" + format + "%'";//varchar
			if(StringUtils.hasText(status)) sql += " and status like '%" + status + "%'";//varchar
			if(StringUtils.hasText(viewCount)) sql += " and view_count=" + viewCount;//int
		} else {
			sql += wheresql;
		}
		...
		MetaApiService service = MetaApi.service();
