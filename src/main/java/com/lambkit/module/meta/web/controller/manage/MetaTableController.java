 ...
 * @since 1.0
 */
public class MetaTableController extends LambkitController {
	private final String tableName = "meta_table";
	...
	public void index() {
		...
		if(id!=null) {
			setAttr("model", MetaTable.service().findById(id));
			setAttr("fields", MetaField.service().find(MetaField.sql().andTbidEqualTo(id).example()));
		}
		keepPara();
	...
	public void list() {
		...
		setAttr("dbs", MetaTable.service().find(MetaTable.sql().example()));
		keepTable(tbc);
		keepPara();
	...
	public void save() {
		MetaTable app = getModel(MetaTable.class, "model");
		app.save();
	...
	public void delete() {
		Long id = getParaToLong(0);
		if(id!=null) {
			MetaTable.service().deleteById(id);
		}
		renderJson(new AjaxResult(1, "success", null));
	}
