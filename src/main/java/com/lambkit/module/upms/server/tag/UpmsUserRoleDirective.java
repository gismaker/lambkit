 ...
 * upms_user_role标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:upms_user_role信息}
 * @author lambkit
 */
public class UpmsUserRoleDirective extends LambkitTemplateModel {

	@Override
	public void onRender(Environment env, Map params, TemplateModel[] loopVars,
			TemplateDirectiveBody body) throws TemplateException, IOException {
		// TODO Auto-generated method stub
		String userRoleId = get(params, "user_role_id");
		...
		String sql = " from upms_user_role where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			if(StringUtils.hasText(userRoleId)) sql += " and user_role_id=" + userRoleId;//int unsigned
			if(StringUtils.hasText(userId)) sql += " and user_id=" + userId;//int unsigned
			if(StringUtils.hasText(roleId)) sql += " and role_id=" + roleId;//int
		} else {
			sql += wheresql;
		}
		...
		UpmsUserRoleService service = UpmsUserRole.service();
