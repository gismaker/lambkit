 ...
 * upms_favorites标签<br>
 * 参数：{id:主键}
 * 返回值：{entity:upms_favorites信息}
 * @author lambkit
 */
public class UpmsFavoritesDirective extends LambkitTemplateModel {
	...
	public void onRender(Environment env, Map params, TemplateModel[] loopVars,
		...
		String sql = " from upms_favorites where "; 
		if(wheresql == null) {
			sql += " 1=1 ";
			...
			if(StringUtils.hasText(tags)) sql += " and tags like '%" + tags + "%'";//varchar
		...
		UpmsFavoritesService service = UpmsFavorites.service();
