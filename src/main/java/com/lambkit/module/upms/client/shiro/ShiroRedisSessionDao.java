...
public class ShiroRedisSessionDao extends CachingSessionDAO {
    ...
    private static Logger _log = LoggerFactory.getLogger(ShiroRedisSessionDao.class);
    // 会话key
    ...
    private void redisSet(String key, String value, int seconds) {
    ...
    protected Serializable doCreate(Session session) {
        ...
        redisSet(LAMBKIT_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(session), (int) session.getTimeout() / 1000);
    ...
    protected Session doReadSession(Serializable sessionId) {
        String session = Redis.use().get(LAMBKIT_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    protected void doUpdate(Session session) {
        ...
        redisSet(LAMBKIT_SHIRO_SESSION_ID + "_" + session.getId(), SerializableUtil.serialize(session), (int) session.getTimeout() / 1000);
    ...
    protected void doDelete(Session session) {
        ...
        if ("client".equals(upmsType)) {
            ...
            String code = Redis.use().get(LAMBKIT_SHIRO_CLIENT_SESSION_ID + "_" + sessionId);
            Jedis jedis = Redis.use().getJedis();
            jedis.del(LAMBKIT_SHIRO_CLIENT_SESSION_ID + "_" + sessionId);
            jedis.srem(LAMBKIT_SHIRO_CLIENT_SESSION_IDS + "_" + code, sessionId);
            jedis.close();
        }
        if ("server".equals(upmsType)) {
            // 当前全局会话code
            String code = Redis.use().get(LAMBKIT_SHIRO_SERVER_SESSION_ID + "_" + sessionId);
            // 清除全局会话
            Redis.use().del(LAMBKIT_SHIRO_SERVER_SESSION_ID + "_" + sessionId);
            // 清除code校验值
            Redis.use().del(LAMBKIT_SHIRO_SERVER_CODE + "_" + code);
            // 清除所有局部会话
            Jedis jedis = Redis.use().getJedis();
            Set<String> clientSessionIds = jedis.smembers(LAMBKIT_SHIRO_CLIENT_SESSION_IDS + "_" + code);
            for (String clientSessionId : clientSessionIds) {
                jedis.del(LAMBKIT_SHIRO_CLIENT_SESSION_ID + "_" + clientSessionId);
                jedis.srem(LAMBKIT_SHIRO_CLIENT_SESSION_IDS + "_" + code, clientSessionId);
            }
            _log.debug("当前code={}，对应的注册系统个数：{}个", code, jedis.scard(LAMBKIT_SHIRO_CLIENT_SESSION_IDS + "_" + code));
            jedis.close();
            // 维护会话id列表，提供会话分页管理
            Redis.use().lrem(LAMBKIT_SHIRO_SERVER_SESSION_IDS, 1, sessionId);
        }
        // 删除session
        Redis.use().del(LAMBKIT_SHIRO_SESSION_ID + "_" + sessionId);
    ...
    public Map getActiveSessions(int offset, int limit) {
        ...
        Jedis jedis = Redis.use().getJedis();
        // 获取在线会话总数
        long total = jedis.llen(LAMBKIT_SHIRO_SERVER_SESSION_IDS);
        // 获取当前页会话详情
        List<String> ids = jedis.lrange(LAMBKIT_SHIRO_SERVER_SESSION_IDS, offset, (offset + limit - 1));
        List<Session> rows = new ArrayList<>();
        for (String id : ids) {
            String session = Redis.use().get(LAMBKIT_SHIRO_SESSION_ID + "_" + id);
            // 过滤redis过期session
            if (null == session) {
                Redis.use().lrem(LAMBKIT_SHIRO_SERVER_SESSION_IDS, 1, id);
                total = total - 1;
                continue;
            }
    ...
    public int forceout(String ids) {
        ...
        for (String sessionId : sessionIds) {
            ...
            redisSet(LAMBKIT_SHIRO_SESSION_ID + "_" + sessionId, SerializableUtil.serialize(upmsSession), (int) upmsSession.getTimeout() / 1000);
        }
