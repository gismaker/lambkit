...
/**
 * http 反向代理服务 public void proxy_hello() { 
 * render(new ProxyRender("proxy_hello", "http://127.0.0.1:9090/hello")); 
 * } 
 * 通过render返回代理服务的结果
 */
public class ProxyRender extends Render {
	static Log log = Log.getLog(ProxyRender.class);

	protected static final String ATTR_TARGET_URI = ProxyRender.class.getSimpleName() + ".targetUri";
	protected static final String ATTR_TARGET_HOST = ProxyRender.class.getSimpleName() + ".targetHost";
	...
	public ProxyRender(String targetName, String targetUri) {
	...
	public void resetUriAndHost(HttpServletRequest servletRequest) {
		...
		//System.out.println("proxy targetUri: " + targetUri);
		try {
	...
	public ProxyRender renderResponse(HttpResponse httpResponse) {
		service(httpResponse, request, response);
	...
	public String get(List<NameValuePair> params) {
		HttpResponse httpResponse = null;
		String result = null;
		try {
			httpResponse = httpGet(params);
	...
	public HttpResponse httpGet(Map<String, String> params) throws ClientProtocolException, IOException {
		List<NameValuePair> nameValuePairs = new ArrayList<>();
		for (String key : params.keySet()) {
			nameValuePairs.add(new BasicNameValuePair(key, params.get(key)));
		}
		return httpGet(nameValuePairs);
	}
	
	public HttpResponse httpGet(List<NameValuePair> params) throws ClientProtocolException, IOException {
	...
	public HttpResponse httpPost(Map<String, String> params) throws ClientProtocolException, IOException {
		List<NameValuePair> nameValuePairs = new ArrayList<>();
		for (String key : params.keySet()) {
			nameValuePairs.add(new BasicNameValuePair(key, params.get(key)));
		}
		return httpPost(nameValuePairs);
	}
	
	public HttpResponse httpPost(List<NameValuePair> params) throws ClientProtocolException, IOException {
		StringBuffer server_url = new StringBuffer(targetUri);
	...
	public String accessStr(HttpServletRequest servletRequest) {
		return accessStr(servletRequest, null);
	}

	public String accessStr(HttpServletRequest servletRequest, Map<String, String> params) {
		HttpResponse httpResponse = access(servletRequest, params);
		String result = null;
		if (httpResponse.getStatusLine().getStatusCode() == HttpStatus.SC_OK) {
			HttpEntity httpEntity = httpResponse.getEntity();
			try {
	 ...
	 * @return
	 */
	public HttpResponse access(HttpServletRequest servletRequest) {
		return access(servletRequest, null);
	}
	 ...
	 * @return
	 */
	public HttpResponse access(HttpServletRequest servletRequest, Map<String, String> params) {
		resetUriAndHost(servletRequest);
	...
	protected void service(HttpResponse proxyResponse, HttpServletRequest servletRequest, HttpServletResponse servletResponse) {
		resetUriAndHost(servletRequest);
		try {
			int statusCode = proxyResponse.getStatusLine().getStatusCode();
			servletResponse.setStatus(statusCode, proxyResponse.getStatusLine().getReasonPhrase());
